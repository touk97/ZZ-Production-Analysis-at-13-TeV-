...
void PSO_4VAR()
{
  ...
  // Output log file
  ofstream logFile("./4var/PSO_4VAR.txt");
  ...
  for (int i = 0; i < iterations; i++)
  {
    for (int j = 0; j < n_particles; j++)
    {
      ...
      if (n_data[0] == 0)
      {
        cout << "    Position:  (" << particle.position[0] << ", " << particle.position[1] << ", " << particle.position[2] << ", " << particle.position[3] << ")" << endl  << endl;
        cout << "    No Events - Terminate " << endl << endl;
        return;
      ...
      hist2d_njets->Fill(particle.position[1], particle.position[0]);
      hist2d_met->Fill(particle.position[1], particle.position[2]);
      hist2d_metoht->Fill(particle.position[1], particle.position[3]);
    ...
    hist2d_met->GetYaxis()->SetTitle(" E^{miss}_{T}}");
    hist2d_met->GetZaxis()->SetTitle(" Entries");
    hist2d_met->SetTitle(Form("Iterations: %.0d | Particles: %.0d", i+1, n_particles));
    ...
    hist2d_metoht->SetTitle(Form("Iterations: %.0d | Particles: %.0d", i+1, n_particles));
    ...
    hist2d_njets->GetXaxis()->SetTitle(" dMetZphi");
    hist2d_njets->GetYaxis()->SetTitle(" Njets");
    hist2d_njets->GetZaxis()->SetTitle(" Entries");
    hist2d_njets->SetTitle(Form("Iterations: %.0d | Particles: %.0d", i+1, n_particles));
    hist2d_njets->SetStats(0);
    gStyle->SetPalette(kRainBow);
    // hist2d_njets->SetContour(1000);
    hist2d_njets->Draw("COLZ");
    // hist2d_njets->Draw("LEGO2Z");


    c1->SetGrid();
    c1->Draw();
    c1->SaveAs("./4var/significance_vs_iter_4var.png");


    c2->Draw();
    if ((i+1) % 5 == 0)
    {
      string framename = "./4var/frames/met/met_iter_" + to_string(i+1) + ".png"; 
      c2->SaveAs(framename.c_str());
    }
    c2->SaveAs("./4var/search_space_met_4var.png");


    c3->Draw();
    if ((i+1) % 5 == 0)
    {
      string framename = "./4var/frames/metoht/metoht_iter_" + to_string(i+1) + ".png"; 
      c3->SaveAs(framename.c_str());
    }
    c3->SaveAs("./4var/search_space_metoht_4var.png");

    c4->Draw();
    if ((i+1) % 5 == 0)
    {
      string framename = "./4var/frames/njets/njets_iter_" + to_string(i+1) + ".png"; 
      c4->SaveAs(framename.c_str());
    }
    c4->SaveAs("./4var/search_space_njets_4var.png");
